...
namespace OpenCharas
{
	public partial class Canvas
	{
		...
		public static class Settings
		{
			...
			{
				_settings = new SettingsContainer(typeof(Settings));

				_settings.Add
				(
					new SettingProperty("FirstRun"),
					new SettingProperty("DontAskGameChange"),
					new SettingProperty("CurrentGame"),
					new SettingProperty("WindowGeometry_Items"),
					new SettingProperty("WindowGeometry_Canvas"),
					new SettingProperty("WindowGeometry_Layers"),
					new SettingProperty("WindowGeometry_CharSelect")
				);
		...
		public void UpdateDrawing()
		{
			if (ToolStripContainer1.ContentPanel.Width != 0 && ToolStripContainer1.ContentPanel.Height != 0)
			{
				...
				{
					if (Program.characterSelectForm.CharacterList.CurrentCharacter.Character != null)
					{
						foreach (var Layer in Program.characterSelectForm.CharacterList.CurrentCharacter.Character.Layers)
		...
		{
			if (Program.characterSelectForm.CharacterList.CurrentCharacter.Character.Layers.Count == 0)
			{
				...
				using (var InFile = System.IO.File.Open(Path, System.IO.FileMode.Open))
				{
					...
					if (Reader != null)
					{
						...
						else if (Header == EHeaderValue.HeaderSheetNoImages || Header == EHeaderValue.HeaderSheetImages)
						{
							...
							Program.characterSelectForm.CharacterList.CurrentCharacter = Program.characterSelectForm.TreeView1.Nodes[0].Nodes[0] as RPGCharacterNode;
							UpdateDrawing();
		...
		public void ExportSprite()
		{
			using (SaveFileDialog SaveDlg = new SaveFileDialog())
			{
				...
				if (Result == System.Windows.Forms.DialogResult.OK)
				{
					Bitmap Image = RenderToTexture(Program.characterSelectForm.CharacterList.CurrentCharacter.Character, SaveDlg.FilterIndex == 2);
		...
		{
			Program.characterSelectForm.CharacterList.CurrentCharacter.Character.Layers.Clear();
		...
		public void ResetEverything()
		{
			...
			Program.characterSelectForm.ClearCharacters();
			Program.characterSelectForm.CharacterList.CurrentCharacter = (RPGCharacterNode)(Program.characterSelectForm.TreeView1.Nodes[0].Nodes[0]);
			Program.layersWindowForm.TreeView1.Nodes.Clear();
			Program.layersWindowForm.CurrentNode = null;
			Program.layersWindowForm.ChangeStates(false);
		...
		private Point MouseDelta = new Point(0, 0);
		...
		public void PictureBox1_MouseDown(System.Object sender, System.Windows.Forms.MouseEventArgs e)
		{
			MovingCamera = true;
			OldMousePos.X = e.X;
			OldMousePos.Y = e.Y;
		...
		public void PictureBox1_MouseUp(System.Object sender, System.Windows.Forms.MouseEventArgs e)
		{
			MovingCamera = false;
		}

		public void PictureBox1_MouseMove(System.Object sender, System.Windows.Forms.MouseEventArgs e)
		{
			if (MovingCamera)
			{
				MouseDelta = new Point(e.X - OldMousePos.X, e.Y - OldMousePos.Y);
		...
		public void PictureBox1_MouseClick(System.Object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			{
				easyPositionForm = new EasyPosition();
				easyPositionForm.Show();
				easyPositionForm.Location = new Point(MousePosition.X, MousePosition.Y);
				easyPositionForm.TopMost = true;
				easyPositionForm.FormClosed += new FormClosedEventHandler(easyPositionForm_FormClosed);
			}
		}

		void easyPositionForm_FormClosed(object sender, FormClosedEventArgs e)
		{
			easyPositionForm = null;
		}
		...
		{
			using (Windows.GameImageSizeShower giss = new Windows.GameImageSizeShower())
				giss.ShowDialog();
		}
